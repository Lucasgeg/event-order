...
import { Button, Input, Label, IconButton, EmptyState, cn } from "./ui";
...
interface OrderFormProps {
  /** Présent = édition d'une commande existante ; absent = nouvelle commande. */
  orderId?: string;
  /** Appelé après un enregistrement/modification réussi. */
  onDone?: () => void;
  /**
   * "standalone" (défaut) : écran plein autonome avec son propre header
   * (utilisé sur /user). "embedded" : monté dans un écran parent (l'onglet
   * commandes de /admin) qui gère déjà sa propre navigation.
   */
  variant?: "standalone" | "embedded";
}

export function OrderForm({ orderId, onDone, variant = "standalone" }: OrderFormProps) {
  ...
  return (
    <div
      className={
        variant === "standalone"
          ? "min-h-screen bg-cream flex flex-col h-screen overflow-hidden"
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-cream relative">
          {variant === "standalone" ? (
            ...
            <div className="flex items-center gap-3 shrink-0 pb-4">
              <IconButton label="Retour à la liste" onClick={() => onDone?.()}>
